...
const CreateExam = () => {
  ...
  const addNewPart = (type) => {
    ...
    // Tự động scroll xuống cuối khi thêm part mới
    setTimeout(() => {
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className="sticky top-0 z-50 w-full flex flex-col shadow-sm">
          {/* Header Chính */}
          <div className="bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-none">Tạo Đề Thi Mới</h2>
                <p className="text-[10px] sm:text-xs font-semibold text-slate-500 mt-1 uppercase tracking-widest hidden sm:block">
                  Dynamic Builder
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-5 sm:px-6 py-2 sm:py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white rounded-xl text-xs sm:text-sm font-bold shadow-md shadow-blue-200 disabled:opacity-60 transition-all"
              >
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                <span className="hidden sm:inline">Lưu bộ đề</span>
                <span className="sm:hidden">Lưu</span>
              </button>
            </div>
          </div>

          {/* Toolbox Thêm Part (Chỉ hiện khi ở tab Builder) - Dính chặt dưới Header */}
          {activeTab === 'builder' && (
            <div className="bg-slate-50/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-8 py-3 flex items-center justify-between overflow-x-auto shadow-inner no-scrollbar">
              <div className="flex items-center gap-2 flex-shrink-0 mr-4">
                <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white shadow-sm">
                  <Plus className="w-4 h-4" />
                </div>
                <span className="text-sm font-black text-slate-800 hidden sm:inline">Thêm Phần Thi:</span>
              </div>
              <div className="flex items-center gap-2">
                {PART_TYPES.map(pt => (
                  <button 
                    key={pt.type} onClick={() => addNewPart(pt.type)} 
                    className={`flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-[11px] sm:text-xs font-bold border transition-all hover:scale-105 active:scale-95 whitespace-nowrap ${pt.color}`}
                  >
                    <pt.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {pt.label}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-8 py-8">
            ...
            {activeTab === 'builder' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
                ...
                {form.parts.length === 0 ? (
                  <div className="text-center py-24 bg-white border-2 border-dashed border-slate-200 rounded-[2rem] shadow-sm mt-4">
                    ...
                    {form.parts.map((part, index) => (
                      <div key={part.id} className="relative group bg-white rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <div className="absolute -left-4 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                          <button onClick={() => movePart(index, 'up')} disabled={index === 0} className="p-1.5 bg-white border border-slate-200 rounded-full text-slate-400 hover:text-blue-600 hover:bg-blue-50 disabled:opacity-30 shadow-sm transition-colors">
                            <ChevronUp className="w-4 h-4" />
                          </button>
                          <button onClick={() => movePart(index, 'down')} disabled={index === form.parts.length - 1} className="p-1.5 bg-white border border-slate-200 rounded-full text-slate-400 hover:text-blue-600 hover:bg-blue-50 disabled:opacity-30 shadow-sm transition-colors">
