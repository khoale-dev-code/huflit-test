...
// ✅ Đã thêm ChevronDown vào danh sách import
import { Star, ChevronUp, ChevronDown, Zap, X, CheckCircle2, Edit2, Trash2, Music, Plus, Check, Volume2 } from 'lucide-react';
...
export const QuestionItem = ({ 
  ...
  // ─── VIEW MODE ───
  return (
    <div className={`rounded-3xl p-5 sm:p-6 border-2 border-b-[6px] transition-all group relative font-nunito ${
        question.isExample ? 'bg-[#FFFBEA] border-[#FFD8A8] hover:border-[#FFC200]' : 'bg-white border-slate-200 hover:border-slate-300'
      }`}
    >
      {/* ✅ BAR CÔNG CỤ NỔI BẬT LÚC RÊ CHUỘT VÀO (Lên, Xuống, Sửa, Xóa) */}
      <div className="absolute right-4 top-4 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1.5 bg-white p-1.5 rounded-[18px] shadow-sm border-2 border-slate-200 z-10">
        ...
        {onMoveDown && (
          <button
            ...
            disabled={index === totalQuestions - 1} // Khóa lại nếu là câu cuối cùng
            className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#1CB0F6] hover:bg-[#EAF6FE] rounded-xl transition-all disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400 outline-none"
            title="Chuyển xuống"
          >
        ...
        <button
          ...
          className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#FF9600] hover:bg-[#FFFBEA] rounded-xl transition-all outline-none active:scale-95"
        ...
        <button
          ...
          className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#FF4B4B] hover:bg-[#fff0f0] rounded-xl transition-all outline-none active:scale-95"
          title="Xóa câu hỏi"
        >
