...
// ✅ NEW: Hook to detect reduced motion preference
...
const styleTag = `
  ...
  /* ✅ CRITICAL: Disable animations on mobile */
  ...
  /* ✅ CRITICAL: Respect user's motion preference */
...
const OrbitCard = ({ icon: Icon, title, subtitle, className, delayClass, animate = true }) => (
  <div className={`
  ...
  `}>
    ...
    <div>
      <p className="text-xs font-bold text-slate-600 uppercase tracking-wider">{subtitle}</p>
  ...
  return (
    <div className="relative w-full min-h-[85vh] flex flex-col items-center justify-center overflow-hidden bg-slate-50 font-sans selection:bg-blue-200">
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
        ...
        {/* ✅ Các quả cầu màu di chuyển (Blobs) - Disabled trên mobile */}
        {!prefersReducedMotion && (
          <>
            <div className="absolute top-0 left-1/4 w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob hidden md:block" />
            <div className="absolute top-0 right-1/4 w-96 h-96 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000 hidden md:block" />
            <div className="absolute -bottom-32 left-1/3 w-96 h-96 bg-cyan-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-4000 hidden md:block" />
          </>
        )}
      </div>
      ...
      <div className="relative z-10 text-center max-w-4xl px-4 mt-16 sm:mt-0">
        ...
        {/* Tagline Badge */}
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-sm font-bold mb-8 ${!prefersReducedMotion ? 'animate-float-slow' : ''}`}>
        ...
        <p className="text-lg sm:text-xl text-slate-700 max-w-2xl mx-auto mb-10 leading-relaxed">
        ...
        <div className="relative group inline-block">
          <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl blur opacity-30 group-hover:opacity-70 transition duration-200 hidden md:block" aria-hidden="true"></div>
          <button className="relative flex items-center gap-3 px-8 py-4 bg-white rounded-xl leading-none text-slate-900 font-bold border border-slate-200 hover:bg-slate-50 transition-all active:scale-95 shadow-xl">
            ...
            <ChevronRight className="w-5 h-5 text-slate-400 group-hover:translate-x-1 transition-transform" aria-hidden="true" />
          </button>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-slate-200/60 flex flex-wrap justify-center gap-8 sm:gap-16">
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-slate-700 uppercase tracking-wide">Đề thi</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-slate-900">24/7</p>
            <p className="text-sm font-semibold text-slate-700 uppercase tracking-wide">Hỗ trợ</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-slate-900">Top 1</p>
            <p className="text-sm font-semibold text-slate-700 uppercase tracking-wide">Uy tín</p>
          </div>
        </div>
      ...
      {/* ✅ Chỉ hiển thị trên lg+ screens và khi không có motion preference */}
