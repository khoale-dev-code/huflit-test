...
import { 
  ...
  Trophy,
  FileText,
  Sparkles // Dùng Sparkles để cá nhân hóa
} from 'lucide-react';
...
const styleTag = `
  ...
  .animate-float-delayed { animation: float-slow 5s ease-in-out infinite reverse; }
`;
...
const HeaderSection = ({ isSignedIn, user, onStartClick }) => {
  ...
  // Danh sách stats dùng useMemo để tối ưu
  const stats = useMemo(() => [
  ...
  return (
    // Sử dụng isScrolled để thay đổi độ mờ của nền khi cuộn
    <div className={`relative w-full overflow-hidden selection:bg-blue-200 min-h-[85vh] flex flex-col items-center justify-center pt-24 pb-16 transition-colors duration-500 ${isScrolled ? 'bg-white' : 'bg-[#F4F7FA]'}`}>
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto px-4">
        ...
        >
          ...
          <span className="relative inline-block mt-2">
            ...
            <svg className="absolute -bottom-3 left-0 w-full h-4 text-[#FFC800] z-0 opacity-80" viewBox="0 0 100 10" preserveAspectRatio="none">
