...
import {
  ...
  Sparkles, ChevronDown, Lightbulb, Send, Image as ImageIcon, FileText, Star
} from 'lucide-react';
...
const ProgressBar = memo(({ percentage }) => (
  <div className="h-2.5 w-full rounded-full overflow-hidden border-[1.5px]" style={{ background: C.n200, borderColor: C.n200 }}>
    <div className="h-full rounded-full relative overflow-hidden transition-all duration-700 ease-out" style={{ width: `${percentage}%`, background: percentage === 100 ? C.green : C.blue }}>
      <div className="absolute top-0.5 left-1 right-1 h-[3px] rounded-full" style={{ background: 'rgba(255,255,255,0.35)' }} />
    </div>
  </div>
));
...
const ScriptDisplay = memo(({ script }) => {
  ...
  return (
    <div className="mb-4 rounded-[20px] border-2 overflow-hidden transition-all shadow-sm" style={{ background: C.blueBg, borderColor: C.blueBorder }}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-4 py-3 bg-transparent border-none cursor-pointer outline-none hover:bg-white/40 transition-colors">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-[12px] border-2 flex items-center justify-center shrink-0" style={{ background: '#DAEEFB', borderColor: C.blueBorder, boxShadow: `0 3px 0 ${C.blueBorder}` }}>
            <Volume2 size={18} color={C.blue} strokeWidth={2.5} />
          </div>
          <span className="text-[12px] font-black uppercase tracking-wider pt-0.5" style={{ fontFamily: F.display, color: C.blue }}>Nội dung âm thanh</span>
        </div>
...
const QuestionOption = memo(({ option, index, isSelected, onSelect, questionId, disabled, isCorrectExample }) => {
  ...
  return (
    <label className={`block mb-2 last:mb-0 ${disabled ? 'cursor-default' : 'cursor-pointer'} ${disabled && !isCorrectExample ? 'opacity-60' : 'opacity-100'}`}>
      ...
      <div className="flex items-center gap-3.5 px-4 py-3 rounded-[18px] border-2 transition-all duration-150 active:translate-y-1" style={{
        ...
        <div className="w-9 h-9 rounded-[12px] shrink-0 flex items-center justify-center text-[14px] font-black border-2 transition-colors" style={{
...
/* ─── Question Card ─── */
const QuestionCard = memo(({ question, globalIndex, options, selectedAnswer, onAnswerSelect }) => {
  const isExample = question.isExample;
  
  return (
    <article className="p-5 mb-5 rounded-[28px] border-2 border-b-[6px] shadow-sm overflow-hidden" style={{
      ...
      {question.audioUrl && <MiniAudioPlayer audioUrl={question.audioUrl} />}
      ...
      <div className="flex items-start gap-4 mb-5">
        <div className="w-[38px] h-[38px] rounded-[13px] shrink-0 flex items-center justify-center border-2" style={{
          background: isExample ? '#FF9600' : C.blue, 
          borderColor: isExample ? '#E58700' : C.blueDark,
          boxShadow: `0 3px 0 ${isExample ? '#E58700' : C.blueDark}`
        }}>
          <span className="text-[14px] font-black text-white">{isExample ? 'VD' : globalIndex}</span>
        </div>
      ...
      <div className="flex flex-col gap-1">
        {options?.map((opt, i) => (
          <QuestionOption key={`${question.id}-${i}`} index={i} option={opt} isSelected={selectedAnswer === i} onSelect={() => !isExample && onAnswerSelect(question.id, i)} questionId={question.id} disabled={isExample} isCorrectExample={isExample && question.correct === i} />
        ))}
      </div>
    </article>
  );
...
/* ─── Question Group Card ─── */
const QuestionGroupCard = memo(({ group, globalIndexStart, answers, onAnswerSelect }) => {
  return (
    <div className="mb-8">
      <div className="p-6 mb-5 rounded-[28px] border-2 border-b-[6px] shadow-sm" style={{ background: C.purpleBg, borderColor: C.purpleBorder }}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-[14px] font-black uppercase tracking-wider text-purple-700">Dùng cho câu {globalIndexStart} - {globalIndexStart + (group.subQuestions?.filter(q => !q.isExample).length || 1) - 1}</span>
        </div>

        {group.audioUrl && <div className="mb-4"><MiniAudioPlayer audioUrl={group.audioUrl} /></div>}
        ...
        {group.content && (
          <div className="p-5 bg-white rounded-[20px] border-2 shadow-inner" style={{ borderColor: C.purpleBorder }}>
            <p className="text-[16px] font-bold leading-relaxed m-0 text-slate-700 whitespace-pre-wrap">{group.content}</p>
          </div>
        )}
      </div>

      <div className="pl-4 md:pl-8 border-l-[4px] border-dashed" style={{ borderColor: C.purpleBorder }}>
        {group.subQuestions?.map((sq, idx) => {
          const currentGlobalIndex = globalIndexStart + group.subQuestions.slice(0, idx).filter(q => !q.isExample).length;
          return <QuestionCard key={sq.id} globalIndex={currentGlobalIndex} question={sq} options={sq.options} selectedAnswer={answers[sq.id]} onAnswerSelect={onAnswerSelect} />;
        })}
      </div>
    </div>
  );
  ...
  <div className="mt-6 p-6 rounded-[28px] border-2 border-b-[6px] flex flex-col gap-4 shadow-lg transition-all" style={{ borderColor: isAllAnswered ? C.green : C.n200, borderBottomColor: isAllAnswered ? C.greenDark : C.n200, background: isAllAnswered ? C.greenBg : C.white }}>
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-[13px] font-bold mt-1 mb-0 text-slate-500">{isAllAnswered ? 'Kiểm tra lại một lần nữa trước khi nộp nhé.' : `Bạn vẫn còn ${totalQuestions - answersCount} câu hỏi chưa trả lời.`}</p>
      </div>
    </div>
    <button onClick={onSubmit} className="w-full flex items-center justify-center gap-2.5 py-4 rounded-[20px] text-[16px] font-black uppercase tracking-widest text-white cursor-pointer transition-all active:translate-y-1 active:shadow-none" style={{ background: isAllAnswered ? C.green : C.blue, boxShadow: `0 5px 0 ${isAllAnswered ? C.greenDark : C.blueDark}` }}>
...
const QuestionDisplay = memo(({ selectedPart, selectedExam, partData, answers, onAnswerSelect, showResults, onSubmit }) => {
  ...
  return (
    <div className="relative z-0 min-h-screen pb-24" style={{ background: C.n50, isolation: 'isolate' }}>
      ...
      {/* ── STICKY HEADER (z-20 để nằm dưới Navbar chính z-50) ── */}
        ...
        <div className="max-w-[680px] mx-auto">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-2xl border-2 transition-all" style={{ borderColor: isAllAnswered ? C.green : C.n200, background: isAllAnswered ? C.greenBg : C.white }}>
              ...
              <span className="text-[13px] font-black text-slate-500" style={{ color: isAllAnswered ? C.greenDark : C.n600 }}>{answersCount} / {totalQuestions}</span>
            </div>
          </div>
      ...
      <main className="max-w-[680px] mx-auto px-4 pt-6">
        ...
        {partData.imageUrl && (
          <div className="bounce-in mb-6 p-5 rounded-[28px] border-2 border-b-[6px] flex flex-col items-center gap-3 bg-white shadow-sm" style={{ borderColor: C.n200 }}>
             <div className="flex items-center gap-2 w-full mb-1">
               ...
               <span className="text-[14px] font-black uppercase text-slate-400">Hình ảnh tổng quát</span>
             </div>
        ...
        {partData.questions.map((q) => {
          if (q.type === 'group') {
            ...
            return <QuestionGroupCard key={q.id} group={q} globalIndexStart={startIndex} answers={answers} onAnswerSelect={onAnswerSelect} />;
          }
          const currentIndex = globalQIndex;
          if (!q.isExample) globalQIndex++;
          return <QuestionCard key={q.id} globalIndex={currentIndex} question={q} options={q.options} selectedAnswer={answers[q.id]} onAnswerSelect={onAnswerSelect} />;
        })}
