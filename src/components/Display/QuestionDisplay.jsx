...
/* ══════════════════════════════════════════════════════
   GLOBAL STYLES & ANIMATIONS (Bổ sung font & keyframes cho Tailwind)
══════════════════════════════════════════════════════ */
const GlobalStyles = () => (
  <style>{`
    ...
    /* Staggered animation cho danh sách câu hỏi */
...
const ProgressBar = memo(({ percentage }) => {
  ...
  return (
    <div className="h-1 w-full bg-[#E1E2EC] rounded-full overflow-hidden relative">
      ...
      />
      {!done && percentage > 0 && <div className="absolute top-0 left-0 h-full w-full animate-shimmer rounded-full" style={{ width: `${percentage}%` }} />}
    </div>
  );
});
...
══════════════════════════════════════════════════════ */
// Dùng chuỗi class Tailwind tĩnh để JIT compiler nhận diện
const OPT_CLASSES = [
...
const QuestionOption = memo(({ option, index, isSelected, onSelect, questionId }) => {
  ...
  return (
    <label className="block cursor-pointer group">
      <input type="radio" className="absolute w-0 h-0 opacity-0" name={`q-${questionId}`} checked={isSelected} onChange={onSelect} />
      <div
        className={`relative overflow-hidden flex items-center gap-3 py-2.5 px-3.5 rounded-2xl border-[1.5px] transition-all duration-200 font-sans-m3
          ${isSelected ? `${theme.border} ${theme.bgSelected} ring-2 ring-offset-0 ${theme.ring}` : 'border-[#C4C6D0] bg-[#FAFAFA] hover:bg-gray-100/80 active:bg-gray-200/60'}`}
      >
        {/* Label badge */}
        <div className={`w-7 h-7 rounded-lg shrink-0 flex items-center justify-center transition-colors duration-200 ${isSelected ? theme.base : 'bg-[#E1E2EC]'}`}>
          {isSelected ? <Check size={14} color="#fff" strokeWidth={2.5} /> : <span className="text-sm font-medium text-[#44464F]">{label}</span>}
        </div>
  ...
  return (
    <div className="bg-[#FFFBF0] border border-[#F0E0A0] rounded-[28px] overflow-hidden mb-3">
      ...
      >
        ...
        <ChevronDown size={18} className="text-[#A08000] transition-transform duration-200" style={{ transform: open ? 'rotate(180deg)' : 'none' }} />
      </button>
      {open && (
        <div className="px-4 pb-4 pt-1 border-t border-[#F0E0A0] animate-m3-fadein font-sans-m3">
          {[
            ...
            <div key={label} className="flex gap-2.5 mt-3">
              <span className="text-xs font-medium text-[#7E5700] bg-[#FFE082] px-2 py-0.5 rounded-full shrink-0 h-fit">{label}</span>
...
const QuestionDisplay = memo(({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    onSubmit();
    setStatus({ show: true, message: !isSignedIn ? 'Đăng nhập để lưu kết quả' : '✓ Kết quả đã được lưu!', success: !!isSignedIn });
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FF] text-[#1A1B1F] antialiased selection:bg-[#1A56DB]/20">
      ...
      <header className="sticky top-0 z-30 bg-[#F7F8FF]/95 backdrop-blur-md border-b border-[#C4C6D0] px-4 pt-3 pb-2.5 font-sans-m3">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border transition-colors duration-300
            ${isAllAnswered ? 'bg-[#89F8D2] border-[#006C51]/40 text-[#002116]' : 'bg-[#D8E2FF] border-[#1A56DB]/30 text-[#001257]'}`}
          >
      ...
      {/* ── Question List ── */}
      <div className="px-4 py-4 md:px-5 md:py-5 pb-[120px] md:pb-5 max-w-3xl mx-auto">
        ...
        {/* ── Submit Bar (Responsive Mobile & Desktop gộp chung) ── */}
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-[#C4C6D0] shadow-[0_-2px_20px_rgba(26,86,219,0.08)] px-4 py-3 flex items-center justify-between gap-3 font-sans-m3
                      md:static md:mt-2 md:mb-8 md:bg-[#FAFAFA] md:border md:rounded-[28px] md:px-5 md:py-4 md:shadow-[0_1px_6px_rgba(0,0,0,0.04),0_4px_16px_rgba(26,86,219,0.05)]">
          {/* Status Text */}
          <div className="flex flex-col gap-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">{answersCount}/{totalQuestions} câu</span>
              <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${isAllAnswered ? 'bg-[#89F8D2] text-[#002116]' : 'bg-[#D8E2FF] text-[#001257]'}`}>{percentage}%</span>
            </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 md:px-6 md:py-3 rounded-[28px] text-[13px] md:text-sm font-semibold tracking-wide shrink-0 transition-all duration-200 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A56DB]
              ${isAllAnswered 
                ? 'bg-[#1A56DB] text-white hover:bg-[#1446B5] shadow-[0_2px_12px_rgba(26,86,219,0.3)] hover:shadow-[0_4px_18px_rgba(26,86,219,0.38)]' 
                : 'bg-[#2F3036] text-white hover:bg-[#3F4045] shadow-[0_1px_4px_rgba(0,0,0,0.12)] hover:shadow-[0_2px_8px_rgba(0,0,0,0.18)]'}`}
          >
            <Send size={15} />
      ...
      {status.show && (
        ...
        >
          {status.success ? <CheckCircle size={18} className="shrink-0" /> : <AlertCircle size={18} color="#FFCC02" className="shrink-0" />}
