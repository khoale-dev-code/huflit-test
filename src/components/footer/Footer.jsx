...
import {
  Mail, Phone, Facebook, Youtube, MessageCircle,
  BookOpen, Zap, Users, HelpCircle, Send,
  ChevronRight, Heart, TrendingUp, Award, Github,
  Sparkles, Rocket, AlertCircle
} from 'lucide-react';
import { motion as Motion, AnimatePresence } from 'framer-motion';

// --- SUB-COMPONENTS (GAMIFIED) ---

const FooterCard = memo((props) => {
  const { title, children, icon: IconComponent, iconColor, iconBg, borderDark } = props;
  return (
...
const SmartLink = memo((props) => {
  const { label, href, icon: IconComponent } = props;
  return (
    <a
      href={href}
    ...
    >
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-[14px] bg-slate-100 group-hover:bg-[#1CB0F6] flex items-center justify-center transition-colors">
          <IconComponent size={18} className="text-slate-500 group-hover:text-white" strokeWidth={2.5} />
        </div>
...
const SocialButton = memo((props) => {
  const { href, icon: IconComponent, hoverColor, borderHover } = props;
  return (
    <a 
      href={href} 
...
const DiscordForm = memo((props) => {
  const { isSubmitting, discord, setDiscord, message, setMessage, discordStatus, onSubmit } = props;
  return (
