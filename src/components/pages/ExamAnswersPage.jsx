...
const [selectedExam, setSelectedExam] = useState('');
  ...
  // State chứa danh sách đề thi kéo từ Supabase
  ...
  // 2. Lấy chi tiết đề thi khi chọn Đề
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (examData) {
          ...
          setSelectedPart(''); // Reset phần thi khi đổi đề
          setQuestions([]);
        }
      } catch (err) {
  ...
  // 3. Trích xuất câu hỏi ngay lập tức khi chọn Phần thi (Không cần gọi API nữa)
  useEffect(() => {
